...
import org.processmining.xes.extension.id.IdentitiesMissingException;
import org.processmining.xes.extension.id.IdentityConnection;
...
public class XPathEngine {
	...
	private void setLog(XLog log) throws IdentitiesMissingException {
		...
		//get the connection
		try {
			Collection<IdentityConnection> cons = 
				context.getConnectionManager().getConnections(IdentityConnection.class, context, log);
	...
	private void init() throws IOException {
		...
		Set<PluginParameterBinding> plugins = context.getPluginManager().getPluginsAcceptingOrdered(child.getClass(), false, log.getClass(), File.class);
	...
	public List<Pair<XAttributable, XAttributable>> query(String query, String relQuery, boolean throwException) throws SaxonApiException {
		try {
	...
	protected List<Pair<XAttributable, XAttributable>> query(XdmNode startNode, String query, String relQuery) throws SaxonApiException {
	...
	private XAttributable getItem(XdmNode node) {
		...
		if (node.getNodeName().getLocalName().equals("id")) {
			if(node.getAttributeValue(KEYNAME) != null && node.getAttributeValue(KEYNAME).equals(XIdentityExtension.KEY_ID)) {
